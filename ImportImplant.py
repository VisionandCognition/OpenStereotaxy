...
class MoveOperator(Operator):
    bl_idname   = 'AddElectrode'
    bl_label    = 'Add electrode'    
...
class SurgeryPlannerPanel(bpy.types.Panel):
    """Stereotaxic surgical planning tool add-on"""
    bl_space_type   = 'VIEW_3D'
    bl_region_type  = 'TOOLS'
    bl_label        = 'Electrode'         
    bl_context      = 'objectmode'
    bl_category     = 'SurgeryPlanner'
...
# Register
def register():
    bpy.utils.register_class(SurgeryPlannerPanel)
    bpy.utils.register_class(MoveOperator)
    
# Unregister
def unregister():
    bpy.utils.unregister_class(SurgeryPlannerPanel)
